...
const MobileJournal = () => {
  ...
  const saveEntryLocally = async (entry: JournalEntry) => {
    setSaving(true);
    ...
    try {
      ...
      const success = await saveJournalEntry(localEntry);
      
      if (success) {
        loadLocalEntries();
        toast({
          title: "Entry Saved Locally",
          description: "Your journal entry has been saved to your device for offline access.",
          duration: 3000
        });
      } else {
      ...
      toast({
        title: "Save Failed", 
        description: "Could not save entry to device. Please try again.",
        variant: "destructive"
      });
    } finally {
      setSaving(false);
  ...
  const removeEntryLocally = async (entryId: string) => {
    try {
      ...
      if (success) {
        ...
        toast({
          title: "Entry Removed",
          description: "Journal entry removed from local storage."
        });
      }
    } catch (error) {
      toast({
        title: "Remove Failed",
        description: "Could not remove entry from device.",
        variant: "destructive"
      });
    }
  };
  ...
  const getFilteredEntries = () => {
    let entriesToFilter: (JournalEntry | LocalJournalEntry)[] = [];
    
    switch (viewMode) {
      case 'cloud':
        entriesToFilter = entries;
        break;
      case 'local':
        entriesToFilter = localEntries;
        break;
      case 'all':
      default:
        entriesToFilter = [...entries, ...localEntries];
        break;
    }
          ...
          <div className="space-y-4">
            ...
            <div className="flex space-x-2 mb-4">
              ...
              >
                ...
                <span>All ({entries.length + localEntries.length})</span>
              </Button>
              ...
              >
                ...
                <span>Cloud ({entries.length})</span>
              </Button>
              ...
              >
                ...
                <span>Local ({localEntries.length})</span>
              </Button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Button
                variant="outline"
                onClick={() => setShowLocalStorageManager(true)}
                className="h-20 border-2 border-gray-200 hover:border-teal-300 hover:bg-teal-50 rounded-2xl flex flex-col items-center justify-center"
              >
                <Archive className="h-8 w-8 mb-2 text-gray-600" />
                <span className="text-sm font-medium text-gray-700">Manage Storage</span>
              </Button>
            </div>
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3 text-xs text-gray-500">
                          ...
                          {/* Save locally button for cloud entries */}
                          {viewMode !== 'local' && 'user_id' in entry && (
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={(e) => {
                                e.stopPropagation();
                                saveEntryLocally(entry as JournalEntry);
                              }}
                              disabled={saving}
                        ...
                        <div className="flex space-x-1 mt-2">
                          ...
                          {viewMode !== 'cloud' && !('user_id' in entry) && (
